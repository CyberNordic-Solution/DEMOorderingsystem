        ...
        .select("id, menu_id, name, price, category_id, is_active");
         ...
         ordersByTime.forEach((orders, completedAt) => {
           ...
           const tableNames = orders.map(order => {
             const table = (tablesData as Table[])?.find(t => t.id === order.table_id);
           ...
           const totalAmount = itemsForTheseOrders.reduce((sum, item) => sum + item.price, 0);
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-xl font-semibold">支付历史</h1>
        <button 
          onClick={loadHistory}
               ...
               <div className="flex justify-between items-start mb-3">
                 <div>
                   <h3 className="text-lg font-semibold text-black">
                     支付记录 #{record.orderIds.length > 1 ? `${record.orderIds[0]}-${record.orderIds[record.orderIds.length - 1]}` : record.orderIds[0]}
                   </h3>
